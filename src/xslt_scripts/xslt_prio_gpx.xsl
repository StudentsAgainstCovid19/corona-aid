...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template name="priority_name_template">
        ...
        <xsl:param name="called"></xsl:param>
        <xsl:choose>
    ...
    <xsl:template match="/">
        <gpx xmlns="http://www.topografix.com/GPX/1/1" version="1.1" creator="Wikipedia"
            ...
            <xsl:for-each select="infected/person">
                <wpt>
                    <xsl:attribute name="lon"><xsl:value-of select="lon"></xsl:value-of></xsl:attribute>
                    <xsl:attribute name="lat"><xsl:value-of select="lat"></xsl:value-of></xsl:attribute>
                    <extensions>
                        <id><xsl:value-of select="id"></xsl:value-of></id>
                        <priority><xsl:value-of select="priority"></xsl:value-of></priority>
                        <done><xsl:value-of select="done"></xsl:value-of></done>
                        <called><xsl:value-of select="calledbool"></xsl:value-of></called>
                        <type>
                            <xsl:call-template name="priority_name_template">
                                <xsl:with-param name="prio"><xsl:value-of select="priority"></xsl:value-of></xsl:with-param>
                                <xsl:with-param name="called"><xsl:value-of select="calledbool"></xsl:value-of></xsl:with-param>
                            </xsl:call-template>
                        </type>
                    </extensions>
                </wpt>
